...
def fallout(piece):
    if piece.find("fallout") == -1 and piece.find("辐射") == -1:
        return False
    else:
        return True
...
class Ximalaya_dengIE(XimalayaBaseIE):
    IE_NAME = 'ximalaya'
    IE_DESC = '喜马拉雅FM'
...
class XimalayaAlbum_dengIE(XimalayaBaseIE):
...
# 怎么添加进去
class Ximalaya_deng_X_IE(XimalayaBaseIE):
    IE_NAME = 'ximalaya:renwenjp'
    IE_DESC = 'wo qu'
    _VALID_URL = r'https?://(?:www\.|m\.)?ximalaya\.com.+renwenjp.+'
    _TEMPLATE_URL = '%s://www.ximalaya.com/%s/renwenjp/%s/'
    _LIST_VIDEO_RE = r'<a[^>]+?href="(?P<url>/%s/sound/(?P<id>\d+)/?)"[^>]+?title="(?P<title>[^>]+)">'
    _TESTS = [{
        'url': 'https://www.ximalaya.com/renwenjp/221727/p4/',
        'info_dict': {
            'title': '唐诗三百首（含赏析）',
            'id': '5534601',
        },
    ...
    def _real_extract(self, url):
        ...
        self.downloadX(url, 0)
        for index in list(range(1, 20)):
            # p11/
            src = url + "p" + str(index) + "/"
    ...
    def downloadX(self, src, index):
        ...
        resultFu = list(map(lambda x: x[1], re.findall(r'\"title\":(.*?)\"(.*?)\"', webpage)))
        # pprint(resultFu)
        resultX = filter(poem, resultFu)
        # print("resultX")
        xxx = list(resultX)
        if xxx:
...
# 怎么添加进去
class Ximalaya_deng_X_QuIE(XimalayaBaseIE):
    IE_NAME = 'ximalaya:erciyuan'
    IE_DESC = 'wo qu'
    _VALID_URL = r'https?://(?:www\.|m\.)?ximalaya\.com.+erciyuan.+'
    _TEMPLATE_URL = '%s://www.ximalaya.com/%s/erciyuan/%s/'
    _LIST_VIDEO_RE = r'<a[^>]+?href="(?P<url>/%s/sound/(?P<id>\d+)/?)"[^>]+?title="(?P<title>[^>]+)">'
    _TESTS = [{
        'url': 'https://www.ximalaya.com/erciyuan/221727/p4/',
        'info_dict': {
            'title': '唐诗三百首（含赏析）',
            'id': '5534601',
        },
    ...
    def downloadX(self, src, index):
        ...
        resultFu = list(map(lambda x: x[1], re.findall(r'\"title\":(.*?)\"(.*?)\"', webpage)))
        # pprint(resultFu)
        resultX = filter(fallout, resultFu)
        # print("resultX")
        xxx = list(resultX)
        if xxx:
            print("page:    " + str(index) + "  :  ")
...
class ZhiHuIE(InfoExtractor):
    ...
    IE_NAME = 'zhihu: blog'
    IE_DESC = 'wo qu'
    _VALID_URL = r'https?://(?:www\.|m\.)?zhihu\.com.+posts?.+'
    _TEMPLATE_URL = '%s://www.zhihu.com/%s/posts/%s/'
    _LIST_VIDEO_RE = r'<a[^>]+?href="(?P<url>/%s/sound/(?P<id>\d+)/?)"[^>]+?title="(?P<title>[^>]+)">'
    ...
    def _real_extract(self, url):
        ...
        jsonElement = r.html.find('#js-initialData') 
